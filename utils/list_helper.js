...
const mostBloggedAuthor = (blogs) => {
  ...
  const authorWithBlogs = blogs.reduce((authorWithBlogs, blog) => {
    ...
    authorWithBlogs[blog.author].hasOwnProperty('blogs')
      ? authorWithBlogs[blog.author].blogs++
      : (authorWithBlogs[blog.author].blogs = 1)
    return authorWithBlogs
  }, {})
  console.log(authorWithBlogs)
  const mostBloggedAuthor = Object.keys(authorWithBlogs).reduce(
...
const mostLikedAuthor = (blogs) => {
  ...
  const authorWithBlogs = blogs.reduce((authorWithBlogs, blog) => {
    authorWithBlogs[blog.author] = authorWithBlogs[blog.author] || {}
    authorWithBlogs[blog.author].author = blog.author
    authorWithBlogs[blog.author].hasOwnProperty('likes')
      ? (authorWithBlogs[blog.author].likes += blog.likes)
      : (authorWithBlogs[blog.author].likes = blog.likes)
    return authorWithBlogs
  }, {})

  const mostLikedAuthor = Object.keys(authorWithBlogs).reduce(
